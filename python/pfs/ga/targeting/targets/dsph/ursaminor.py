...
class UrsaMinor(Galaxy):
    ...
    def assign_priorities(self, catalog: Catalog, mask=None):
        ...
        if 'prob' in catalog.data:
            prob = catalog.data['prob'][mask]
            priority = np.full(prob.shape, -1, dtype=np.int32)
        ...
        catalog.data['priority'][mask] = priority

        catalog.data['exp_time'] = np.nan
        catalog.data['exp_time'][mask] = 1800 * np.maximum(np.minimum(np.rint(5 * ((i0 - 16) / (23.0 - 16.0)) + 1).astype(int), 6), 1)