...
class UrsaMinor(DSphGalaxy):
    ...
    def assign_priorities(self, catalog: Catalog, mask=None, isogrid=None, isochrones_name_mappings=None):
        return self.assign_priorities_v2(catalog, mask=mask, isogrid=isogrid, isochrones_name_mappings=isochrones_name_mappings)

    def assign_priorities_v2(self, catalog: Catalog, mask=None, isogrid=None, isochrones_name_mappings=None):
        """
        ...
        # Priorities
        priority = np.full_like(p_member, -1, float)
        ...
        mask_prob = (priority == -1) & np.isfinite(p_member)
        mask_hiprob = mask_prob & (p_member > 0.7)
        mask_loprob = mask_prob & (p_member > 0.0) & (p_member <= 0.7)

        # Priority 1: bright, high-probability RGB
        w1 = mask_hiprob & (g0 < 19.5)
        priority[w1] = 1
        logger.info(f'{(keep & w1).sum()} {self.name} stars are marked as priority 1')

        # Priority 2: medium-bright, high-probability RGB
        w2 = mask_hiprob & (g0 < 21.0) & (g0 >= 19.5)
        priority[w2] = 2
        logger.info(f'{(keep & w2).sum()} {self.name} stars are marked as priority 2')

        # Priority 2.1: faint, high-probability RGB
        w21 = mask_hiprob & (g0 < 22.5) & (g0 >= 21.0)
        priority[w21] = 2.1
        logger.info(f'{(keep & w21).sum()} {self.name} stars are marked as priority 2.1')

        # Priority 2.2: bright, low-probability RGB
        w22 = mask_loprob & (g0 < 19.5)
        priority[w22] = 2.2
        logger.info(f'{(keep & w22).sum()} {self.name} stars are marked as priority 2.2')

        # Priority 2.3: medium-bright, low-probability RGB
        w23 = mask_loprob & (g0 < 21.0) & (g0 >= 19.5)
        priority[w23] = 2.3
        logger.info(f'{(keep & w23).sum()} {self.name} stars are marked as priority 2.3')

        # Priority 2.4: faint, low-probability RGB
        w24 = mask_loprob & (g0 < 22.5) & (g0 >= 21.0)
        priority[w24] = 2.4
        logger.info(f'{(keep & w24).sum()} {self.name} stars are marked as priority 2.4')
    ...
    def assign_priorities_v1(self, catalog: Catalog, mask=None, isogrid=None, isochrones_name_mappings=None):
        """
        Assign priority classes based on photometry
        """
