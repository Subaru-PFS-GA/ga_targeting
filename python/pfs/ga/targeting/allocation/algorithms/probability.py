...
class Probability(TargetingAlgorithm):
    def __init__(self, reverse=None, orig=None):
        super().__init__(orig=orig)

        if not isinstance(orig, Probability):
            self.__reverse = reverse or False
        else:
            self.__reverse = reverse or orig.__reverse

    def argsort_assoc(self, lp_member, assoc: Associations, mask=None, reverse=None):
        """
        Given the fiber associations, pick the object with highest membership probability.
        """
        mask = mask if mask is not None else np.full(lp_member.shape, True, dtype=bool)
        ...
        ix = [ np.argsort(lp_member[mask][a])[s] for a in assoc.assoc ]
